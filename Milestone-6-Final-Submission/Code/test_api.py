...
def admin_login():
  data = {'email': 'admin@iitm.ac.in', 'password': 'P@&&w0rd'}
  requests.post(root + '/', json=data)


def support_login():
  data = {'email': 'support@iitm.ac.in', 'password': 'P@&&w0rd'}
  requests.post(root + '/', json=data)


def student_login():
  data = {'email': 'student@iitm.ac.in', 'password': 'P@&&w0rd'}
  requests.post(root + '/', json=data)


def user_login():
  data = {'email': 'user@iitm.ac.in', 'password': 'P@&&w0rd'}
  requests.post(root + '/', json=data)


def logout():
  requests.get(root + '/logout')
...
class TestHomeAPI():
  ...
  def test_home_post_register_200(self):
    data = {'email': 'user@iitm.ac.in', 'password': 'P@&&w0rd', 'role': 2}
  ...
  def test_home_post_login_400(self):
    data = {'email': 'user@iitm.ac.in', 'password': 'P@ssw0rd'}
    response = requests.post(root + '/', json=data)
    assert response.status_code == 400

  def test_home_post_login_404(self):
    data = {'email': 'users@iitm.ac.in', 'password': 'P@&&w0rd'}
    response = requests.post(root + '/', json=data)
...
class TestUserAPI():
  ...
  def test_user_get_one_200(self):
    admin_login()
    response = requests.get(root + '/user/user@iitm.ac.in')
    assert response.status_code == 200
  ...
  def test_user_post_200(self):
    data = {'email': 'user0@iitm.ac.in', 'password': 'P@&&w0rd', 'role': 2}
  ...
  def test_user_put_401(self):
    logout()
    response = requests.put(root + '/user/user@iitm.ac.in')
    assert response.status_code == 401

  def test_user_put_404(self):
    admin_login()
    response = requests.put(root + '/user/users@iitm.ac.in')
    assert response.status_code == 404

  def test_user_put_password_200(self):
    user_login()
    data = {'password': 'P@&&w0rd'}
    response = requests.put(root + '/user/user@iitm.ac.in', json=data)
    assert response.status_code == 200
  ...
  def test_user_delete_401(self):
    logout()
    response = requests.delete(root + '/user/user@iitm.ac.in')
    assert response.status_code == 401

  def test_user_delete_404(self):
    user_login()
    response = requests.delete(root + '/user/users@iitm.ac.in')
    assert response.status_code == 404
  ...
  def test_user_delete_admin_200(self):
    admin_login()
    response = requests.delete(root + '/user/user0@iitm.ac.in')
    assert response.status_code == 200

  def test_user_delete_user_401(self):
    support_login()
    response = requests.delete(root + '/user/student@iitm.ac.in')
    assert response.status_code == 401
